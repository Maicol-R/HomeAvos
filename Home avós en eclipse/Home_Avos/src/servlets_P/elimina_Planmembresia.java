...
@WebServlet("/elimina_Planmembresia")
public class elimina_Planmembresia extends HttpServlet {
     ...
     DAO_Planmembresia_Impl a=new DAO_Planmembresia_Impl();        
        try {
    		...
    		cond=busqueda.get(0).getcodPlan();
    			...
    			a.eliminar(codPlan);
                String alert=" PLAN de MEMBRESIA '"+busqueda.get(0).getcodPlan()+"' eliminado EXITOSAMENTE";
        	...
        	String alert="ERROR no se pudo eliminar el PLANMEMBRESIA deseado";
        	session.setAttribute("alert", alert);
        	System.out.println("Error "+e);
        	RequestDispatcher r = request.getRequestDispatcher("P_eliminar.jsp");
            r.forward(request, response);    
        } finally {
